...
this measure is sometimes called also as the permutation importance. In order to represent MDA measure mathematically, 
consider an explanatory $j$-th variable $X_{j}$ and stand $OOB_{b,n}$ as a symbol for the out-of-bag data set of 
the $b$-th tree and  $OOB_{b,n}^{j}$ as the same data set, but with randomly permuted values of $X_{j}$. 
Denote also by $ m_{n}(\cdot ;\theta_{b}) $  the $k$-th tree estimate. 
Then definition of MDA is given by:

% NOTATION NOT CLEAR

\begin{equation}
\widehat{MDA}( X_{j} ) = \frac{1}{N_{T}}\displaystyle\sum_{T} \Big[C_{n}\big[m_{n}(\cdot ;\Theta_{k}), OOB_{k,n}^{j}\big] - C_{n}\big[m_{n}(\cdot ;\Theta_{k}), OOB_{k,n}\big]   \Big],
\end{equation}
